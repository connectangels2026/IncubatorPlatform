      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-5 mb-10">
          {stats.map((stat, idx) => (
            <div
              key={idx}
              className="p-5 rounded-xl bg-white border border-slate-200 shadow-sm hover:shadow-md hover:border-slate-300 transition group"
            >
              <div className={`inline-flex p-2.5 rounded-lg ${stat.iconBg} mb-3 group-hover:scale-110 transition-transform`}>
                ...
                <div key={idx} className="flex-1 flex flex-col items-center gap-2 h-full justify-end group">
                  ...
                  <div
                    className={`w-full rounded-t-md transition-all duration-700 ${
                      idx === applicationsTrend.length - 1
                        ? 'bg-gradient-to-t from-blue-500 to-cyan-400'
                        : 'bg-blue-500/15 group-hover:bg-blue-500/40'
                    }`}
                    style={{ height: `${(val / maxApp) * 100}%` }}
                  ></div>
                  <span className="text-[10px] text-slate-400 hidden sm:block">{months[idx]}</span>
                </div>
              ))}
            </div>
          </div>
                ...
                >
                  ...
                  <div className="text-right flex-shrink-0">
                    <div className={`text-xs font-semibold ${
                      item.priority === 'High' ? 'text-rose-600' : item.priority === 'Medium' ? 'text-amber-600' : 'text-slate-500'
                    }`}>
                      {item.priority}
                    </div>
